...
export async function sha256Hex(data: Uint8Array): Promise<string> {
  const subtle: SubtleCrypto | undefined =
    (globalThis.crypto && globalThis.crypto.subtle) || undefined;
  if (subtle) {
...
export class ChunkAssembler {
  ...
  private received: Uint8Array; // 1 byte per chunk flag, avoids Set overhead
  ...
  isComplete(): boolean {
    for (let i = 0; i < this.received.length; i++) {
      if (this.received[i] === 0) return false;
    }
    return true;
  }
  ...
  missingIndices(): number[] {
    ...
    for (let i = 0; i < this.received.length; i++) {
      if (this.received[i] === 0) missing.push(i);
    }
    return missing;
  }
  ...
  progress(): { received: number; total: number } {
    let received = 0;
    for (let i = 0; i < this.received.length; i++) received += this.received[i];
