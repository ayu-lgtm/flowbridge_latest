...
export type ConnectionStatus =
  | "idle"
  | "waiting-for-code" // we created a code, waiting for peer to enter it
  | "pairing" // we submitted a code, waiting for server ack
  | "connecting" // paired, establishing WebRTC
  | "connected"
  | "disconnected"
  | "error";
...
export class FlowConnection {
  ...
  private sessionId: string | null = null; // the permanent pairing code, once known
  private peer: DeviceInfo | null = null;
  private events: ConnectionEvents;
  private transport: TransportKind = "none";
  private isOfferer = false;
  private reconnectAttempts = 0;
  private manuallyClosed = false;
  private pendingIce: RTCIceCandidateInit[] = [];
  private heartbeatTimer: ReturnType<typeof setInterval> | null = null;
  private waitingRetryTimer: ReturnType<typeof setInterval> | null = null;
  private reconnectTimer: ReturnType<typeof setTimeout> | null = null;
  private lastPongAt = Date.now();
  private slowWakeWarned = false;
  ...
  private outbox: FlowMessage[] = [];
  private static readonly MAX_OUTBOX = 400; // ~25MB worst case at default chunk size
  ...
  private static readonly MAX_RECONNECT_DELAY_MS = 20_000;
  private static readonly SLOW_WAKE_WARNING_AFTER_MS = 45_000;
  ...
  private enqueue(msg: FlowMessage): void {
  ...
  private wsSend(msg: FlowMessage) {
  ...
  private startWaitingRetry() {
    if (this.waitingRetryTimer) return;
  ...
  private stopWaitingRetry() {
    if (this.waitingRetryTimer) clearInterval(this.waitingRetryTimer);
  ...
  private scheduleReconnect() {
    ...
    if (!this.slowWakeWarned && elapsed >= FlowConnection.SLOW_WAKE_WARNING_AFTER_MS) {
      ...
      this.events.onError?.(
        "Still trying to reconnect — if the server was asleep this can take up to a minute."
      );
    }
    if (this.reconnectTimer) clearTimeout(this.reconnectTimer);
  ...
  private handleSignalingMessage(msg: FlowMessage) {
    switch (msg.type) {
      case "pair:code": {
      ...
      case "pair:ok": {
        ...
        if (msg.peer.deviceId) this.peer = msg.peer;
        this.setStatus("connecting");
        ...
        if (this.isOfferer) this.startP2p(true);
        return;
      }
      case "pair:error": {
      ...
      default: {
        ...
        if (this.transport !== "p2p") this.transport = "relay";
  ...
  private startP2p(offerer: boolean) {
    if (this.pc) return; // already attempting
  ...
  private handleRemoteOffer(sdp: string) {
    if (!this.pc) this.startP2p(false);
    const pc = this.pc!;
  ...
  private attachDataChannel(dc: RTCDataChannel) {
    ...
    dc.onmessage = (ev) => {
      try {
        const msg: FlowMessage = JSON.parse(ev.data);
        this.events.onMessage?.(msg);
      } catch {
