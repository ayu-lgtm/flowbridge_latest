...
export interface DeviceInfo {
  deviceId: string; // random, generated on first run, persisted locally
  role: DeviceRole;
  label: string; // e.g. "Pixel 8" or "Corporate Laptop"
}
...
export interface BaseMessage {
  type: MessageType;
  messageId: string; // uuid, unique per envelope (not per logical transfer)
  originId: string; // deviceId of the device that authored the content
  ts: number; // epoch ms
}
...
export interface ClipboardUpdateMessage extends BaseMessage {
  ...
  transferId: string;
  contentHash: string; // sha256 hex of the full text, UTF-8
  totalLength: number; // character length (for UI "Last received: N characters")
  encoding: "utf-8";
}
...
export interface TextStartMessage extends BaseMessage {
  type: "text:start";
  transferId: string;
  contentHash: string; // sha256 of the FULL reassembled text
  totalLength: number; // UTF-8 byte length
  chunkCount: number;
  chunkSizeBytes: number;
  encoding: "utf-8";
  newline: "auto"; // never normalized — original bytes preserved verbatim
}
...
export interface TextChunkMessage extends BaseMessage {
  ...
  data: string; // base64 of the raw UTF-8 byte slice for this chunk
}
...
export interface FileOfferMessage extends BaseMessage {
  ...
  fileSize: number;
  mimeType: string;
  contentHash: string; // sha256 of the whole file
}
...
export interface PairHelloMessage extends BaseMessage {
  type: "pair:hello";
  code: string; // short pairing code, e.g. "82K9-XP4Q"
  device: DeviceInfo;
}
export interface PairCodeMessage extends BaseMessage {
  type: "pair:code";
  code: string; // e.g. "82K9-XP4Q"
  expiresInMs: number;
}
...
export type FlowMessage =
  ...
  | PairResumeMessage
  | PairResumedMessage
  | PairResumeWaitingMessage
  | PairResumeFailedMessage
  | SignalOfferMessage
  | SignalAnswerMessage
  | SignalIceMessage
  | PeerJoinedMessage
